...
def setup_train_args():
    ...
    parser.add_argument('--data_dir', default='D:\PycharmProjects\GPT-2', type=str, required=False, help='数据集加载')
    parser.add_argument('--model_save_dir', default='D:\PycharmProjects\GPT-2', type=str, required=False,
                        help='数据集加载')
    parser.add_argument('--log_dir', default='D:\PycharmProjects\GPT-2', type=str, required=False,
                        help='数据集加载')
...
model_config_path = os.path.join(args.data_dir, 'data/config/model_config_dialogue_small.json')
train_raw_path = os.path.join(args.data_dir,'data/train_0w.txt')
train_tokenized_path = os.path.join(args.model_save_dir,'train_tokenized.txt')
log_path = os.path.join(args.log_dir,'training.log')
dialogue_model_output_path = os.path.join(args.model_save_dir,'dialogue_model/')
mmi_model_output_path = os.path.join(args.model_save_dir,'mmi_model/')
train_mmi_tokenized_path = os.path.join(args.model_save_dir,'train_mmi_tokenized.txt')
...
def preprocess_raw_data(args, tokenizer, n_ctx):
    ...
    :return:
    """
    logger.info("tokenizing raw data,raw data path:{}, token output path:{}".format(train_raw_path,
    ...
    with open(train_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            ...
            if dialogue_index < len(train_data) - 1:
                f.write("\n")
    logger.info("finish preprocessing raw data,the result is stored in {}".format(train_tokenized_path))
...
def preprocess_mmi_raw_data(args, tokenizer, n_ctx):
    ...
    :return:
    """
    logger.info("tokenizing MMI raw data,raw data path:{}, token output path:{}".format(train_raw_path,
                                                                                        train_mmi_tokenized_path))
    with open(train_raw_path, 'rb') as f:
        data = f.read().decode("utf-8")
    if "\r\n" in data:
        train_data = data.split("\r\n\r\n")
        ...
        train_data = data.split("\n\n")
    logger.info("there are {} dialogue in raw dataset".format(len(train_data)))
    with open(train_mmi_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            if "\r\n" in data:
                utterances = dialogue.split("\r\n")
            else:
...
def train(model, device, train_list, multi_gpu, args):
    ...
    for epoch in range(args.epochs):
        ...
        if args.train_mmi:  # 当前训练MMI模型
            model_path = join(mmi_model_output_path, 'model_epoch{}'.format(epoch + 1))
        else:  # 当前训练对话模型
            model_path = join(dialogue_model_output_path, 'model_epoch{}'.format(epoch + 1))
...
def main():
    ...
    # 初始化tokenizer
    vocab_path = os.path.join(args.data_dir, 'data/vocabulary/vocab_small.txt')
    ...
    if args.train_mmi:  # 如果是训练MMI模型
        with open(train_mmi_tokenized_path, "r", encoding="utf8") as f:
            data = f.read()
    else:  # 如果是训练对话生成模型
        with open(train_tokenized_path, "r", encoding="utf8") as f:
            data = f.read()
